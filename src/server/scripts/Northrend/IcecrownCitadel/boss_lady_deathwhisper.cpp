    ...
    public:
        ...
        struct boss_lady_deathwhisperAI : public BossAI
        {
            ...
            void MoveInLineOfSight(Unit * who)
            {
                ...
                if (!_introDone && me->IsWithinDist(who, 112.0f))
                {
                    _introDone = true;
                    Talk(SAY_INTRO_1);
            ...
            void EnterCombat(Unit* who)
            {
                ...
                events.ScheduleEvent(EVENT_BERSERK, MINUTEN_10);
                events.ScheduleEvent(EVENT_DEATH_AND_DECAY, SEKUNDEN_10);
                // phase one only
                ...
                events.ScheduleEvent(EVENT_P1_EMPOWER_CULTIST, urand(SEKUNDEN_20, SEKUNDEN_30), 0, PHASE_ONE);
            ...
            void DamageTaken(Unit* /*damageDealer*/, uint32& damage)
            {
                ...
                if (events.GetPhaseMask() & PHASE_ONE_MASK && damage > me->GetPower(POWER_MANA))
                {
                    ...
                    events.ScheduleEvent(EVENT_P2_FROSTBOLT, urand(SEKUNDEN_10, 12 * IN_MILLISECONDS), 0, PHASE_TWO);
                    events.ScheduleEvent(EVENT_P2_FROSTBOLT_VOLLEY, urand(19 * IN_MILLISECONDS, 21 * IN_MILLISECONDS), 0, PHASE_TWO);
                    events.ScheduleEvent(EVENT_P2_TOUCH_OF_INSIGNIFICANCE, urand(SEKUNDEN_10, SEKUNDEN_20), 0, PHASE_TWO);
                    ...
                    if (IsHeroic())
                    {
                        ...
                        events.ScheduleEvent(EVENT_P2_SUMMON_WAVE, urand(SEKUNDEN_30, SEKUNDEN_60), 0, PHASE_TWO);
                    }
                }
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_DEATH_AND_DECAY:
                            ...
                            events.ScheduleEvent(EVENT_DEATH_AND_DECAY, urand(SEKUNDEN_10, 12 * IN_MILLISECONDS));
                            break;
                            ...
                            for (uint8 i = 0; i < _dominateMindCount; i++)
                                if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true, -SPELL_DOMINATE_MIND_H))
                                    if (target != me->getVictim()) // Aus irgend einem Grund ist es sonst möglich, dass der Tank übernommen wird! NO GO!
                                    {
                                        ...
                                        events.ScheduleEvent(EVENT_DOMINATE_MIND_H, urand(SEKUNDEN_40, 45 * IN_MILLISECONDS));
                                    }
                                    else
                        ...
                        case EVENT_P2_FROSTBOLT:
                            ...
                            events.ScheduleEvent(EVENT_P2_FROSTBOLT, urand(SEKUNDEN_10, 11 * IN_MILLISECONDS), 0, PHASE_TWO);
                            break;
                        ...
                        case EVENT_P2_SUMMON_WAVE:
                            SummonWaveP2();
                            events.ScheduleEvent(EVENT_P2_SUMMON_WAVE, SEKUNDEN_60, 0, PHASE_TWO);
                            break;
                        case EVENT_BERSERK:
            ...
            {
                if (TempSummon* summon = me->SummonCreature(entry, pos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, SEKUNDEN_10))
            ...
            void SpellHitTarget(Unit* target, SpellInfo const* spell)
            {
                if (spell->Id == SPELL_DARK_MARTYRDOM_T)
                {
                    ...
                    if (target->GetEntry() == NPC_CULT_FANATIC)
                        me->SummonCreature(NPC_REANIMATED_FANATIC, pos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, SEKUNDEN_10);
                    else
                        me->SummonCreature(NPC_REANIMATED_ADHERENT, pos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, SEKUNDEN_10);
...
class npc_cult_fanatic : public CreatureScript
{
    public:
        ...
        struct npc_cult_fanaticAI : public ScriptedAI
        {
            ...
            {
                Events.Reset();
                Events.ScheduleEvent(EVENT_FANATIC_NECROTIC_STRIKE, urand(SEKUNDEN_10, 12 * IN_MILLISECONDS));
                Events.ScheduleEvent(EVENT_FANATIC_SHADOW_CLEAVE, urand(14 * IN_MILLISECONDS, 16 * IN_MILLISECONDS));
                Events.ScheduleEvent(EVENT_FANATIC_VAMPIRIC_MIGHT, urand(SEKUNDEN_20, 27 * IN_MILLISECONDS));
                ...
                while (uint32 eventId = Events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_FANATIC_SHADOW_CLEAVE:
                            ...
                            Events.ScheduleEvent(EVENT_FANATIC_SHADOW_CLEAVE, urand(9500, 11 * IN_MILLISECONDS));
                            break;
...
class npc_cult_adherent : public CreatureScript
{
    public:
        ...
        struct npc_cult_adherentAI : public ScriptedAI
        {
            ...
            {
                Events.Reset();
                Events.ScheduleEvent(EVENT_ADHERENT_FROST_FEVER, urand(SEKUNDEN_10, 12 * IN_MILLISECONDS));
...
class npc_darnavan : public CreatureScript
{
    public:
        ...
        struct npc_darnavanAI : public ScriptedAI
        {
            ...
            {
                _events.Reset();
                _events.ScheduleEvent(EVENT_DARNAVAN_BLADESTORM, SEKUNDEN_10);
                _events.ScheduleEvent(EVENT_DARNAVAN_INTIMIDATING_SHOUT, urand(SEKUNDEN_20, 25 * IN_MILLISECONDS));
                _events.ScheduleEvent(EVENT_DARNAVAN_MORTAL_STRIKE, urand(25 * IN_MILLISECONDS, SEKUNDEN_30));
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (_canShatter && me->getVictim()->IsImmunedToDamage(SPELL_SCHOOL_MASK_NORMAL))
                {
                    ...
                    _canShatter = false;
                    _events.ScheduleEvent(EVENT_DARNAVAN_SHATTERING_THROW, SEKUNDEN_30);
                    return;
                }
                ...
                if (_canCharge && !me->IsWithinMeleeRange(me->getVictim()))
                {
                    ...
                    _canCharge = false;
                    _events.ScheduleEvent(EVENT_DARNAVAN_CHARGE, SEKUNDEN_20);
                    return;
                }
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_DARNAVAN_MORTAL_STRIKE:
                            ...
                            _events.ScheduleEvent(EVENT_DARNAVAN_MORTAL_STRIKE, urand(15 * IN_MILLISECONDS, SEKUNDEN_30));
                            break;
