                            ...
                            case 1:
                            {
                                ...
                                for (TPlayerList::iterator it = players.begin(); it != players.end(); ++it)
                                    if ((*it)->HasAura(SPELL_IN_FROSTMOURNE_ROOM))
                                    {
                                        ...
                                        (*it)->CastSpell((*it), FROZEN_THRONE_TELEPORT, true);
...
static const float Z_FLY = 1045.0f;
...
static const float Z_VILE_SPIRIT = 1050.0f;
...
enum ShamblingEvents
{
    EVENT_ENRAGE = 1,
    EVENT_SHOCKWAVE
};

enum ShamblingSpells
{
...
public:
    ...
    struct npc_shadow_trapAI : public Scripted_NoMovementAI
    {
        ...
        void Reset()
        {
            events.ScheduleEvent(EVENT_BECOME_ACTIVE_, 500);
            events.ScheduleEvent(EVENT_DESPAWN_, 60000);
            SetCombatMovement(false);
            active = false;
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_ACTIVE:
                    {
                        ...
                        events.ScheduleEvent(EVENT_DESPAWN_, 45000);
