...
public:
    ...
    static bool HandleGMListFullCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        QueryResult result = LoginDatabase.PQuery("SELECT a.username, aa.gmlevel FROM account a, account_access aa WHERE a.id=aa.id AND aa.gmlevel >= %u AND (aa.realmid = -1 OR aa.realmid = %u)", SEC_ANWAERTER, realmID);
        if (result)
        {
