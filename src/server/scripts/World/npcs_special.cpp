    ...
    struct npc_schutz_den_wehrlosenAI : public ScriptedAI
    {
        ...
        void DamageTaken(Unit * /*victim*/, uint32 & dmg)
        {
            if (enabled)
            {
                ...
                if (!done75 && !me->HasUnitState(UNIT_STATE_CASTING) && me->HealthBelowPct(75))
                {
                    DoCastAOE(SPELL_SCHOCKWELLE);
                    done75 = true;
                }

                if (!done50 && !me->HasUnitState(UNIT_STATE_CASTING) && me->HealthBelowPct(50))
                {
                    DoCastAOE(SPELL_SCHOCKWELLE);
                    done50 = true;
                }

                if (!done25 && !me->HasUnitState(UNIT_STATE_CASTING) && me->HealthBelowPct(25))
                {
                    DoCastAOE(SPELL_SCHOCKWELLE);
                    done25 = true;
                }
