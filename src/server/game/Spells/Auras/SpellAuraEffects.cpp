...
void AuraEffect::HandleAuraDetectAmore(AuraApplication const* aurApp, uint8 /*mode*/, bool apply) const
{
    ...
    if (apply)
        target->SetByteFlag(PLAYER_FIELD_BYTES2, 3, PLAYER_FIELD_BYTE2_DETECT_AMORE_0 << GetMiscValue());
    else
    {
        // do not remove unit flag if there are more than this auraEffect of that kind on unit on unit
        if (target->HasAuraTypeWithMiscvalue(GetAuraType(), GetMiscValue()))
            return;
        target->RemoveByteFlag(PLAYER_FIELD_BYTES2, 3, PLAYER_FIELD_BYTE2_DETECT_AMORE_0 << GetMiscValue());
    }
}
